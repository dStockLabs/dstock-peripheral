/**
 * Revoke ADMIN_ROLE from a previous admin address (no npm deps).
 *
 ...
 *
 * Optional:
 * - TARGET_ADDRESS: defaults to 0x6dc731481648Cd108120151F6ca1CbeA8277cE36
 ...
 *   LEGACY_TX=1 GAS_PRICE=0.05gwei node scripts/revokeAdminRole.js
 */

import { spawnSync } from "node:child_process";

const DEFAULT_TARGET = "0x6dc731481648Cd108120151F6ca1CbeA8277cE36";
...
async function main() {
  ...
  const target = normAddr("TARGET_ADDRESS", process.env.TARGET_ADDRESS || DEFAULT_TARGET);
  ...
  console.log(`TARGET_ADDRESS=${target}`);
  ...
  sendArgs.push(router, "revokeRole(bytes32,address)", role, target);
  run("cast", sendArgs, { dryRun });

  console.log("\nDone.");
  console.log("Optional verify:");
  console.log(`  cast call --rpc-url ${rpcUrl} ${router} "hasRole(bytes32,address)(bool)" ${role} ${target}`);
}
